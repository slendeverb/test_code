...
struct PositionalEncodingImpl : torch::nn::Module {
    ...
    PositionalEncodingImpl(const std::string &module_name = "positional_encoding", int d_model = 0,
                           float dropout_value = 0.1, int max_len = 5000)
    ...
    torch::Tensor forward(torch::Tensor x) {
        //[1, 5000, 100]
        auto peb = pe.index({
            torch::indexing::Slice(0, torch::indexing::None), torch::indexing::Slice(0, x.size(1)),
...
struct TransGANppGeneratorImpl : torch::nn::Module {
    ...
    TransGANppGeneratorImpl(
        ...
        patch_decoder(
            torch::nn::ConvTranspose2dOptions(kNoiseSize, 1, patch_size).stride(patch_size).padding(2).bias(false))
    //100, 4, 4 -> 1*28*28
    {
...
int main(int argc, const char *argv[]) {
    ...
    torch::optim::Adam generator_optimizer(generator->parameters(),
                                           torch::optim::AdamOptions(1e-4).weight_decay(0.001).betas(
                                               std::make_tuple(0.5, 0.99)));
    torch::optim::Adam discriminator_optimizer(discriminator->parameters(),
                                               torch::optim::AdamOptions(2e-4).weight_decay(0.001).betas(
                                                   std::make_tuple(0.5, 0.99)));
    ...
    for (int64_t epoch = 1; epoch <= NUMBER_OF_EPOCHS; epoch++) {
        ...
        for (torch::data::Example<> &batch: *data_loader) {
            ...
            torch::Tensor real_labels = torch::empty(real_output.sizes(), device).uniform_(0.8, 1.0);
            //!!!(0.8, 1.0); label smoothing
            ...
            if (batch_index % LOG_INTERVAL == 0) {
                std::cout << "[" << epoch << "|" << NUMBER_OF_EPOCHS << "][" << batch_index << "|" << batches_per_epoch
                        << "] d_loss: " << d_loss.item<float>() << "; g_loss: " << g_loss.item<float>() << ";" <<
                        std::endl;
            }
